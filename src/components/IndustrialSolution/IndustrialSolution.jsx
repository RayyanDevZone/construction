const cardData = [
  {
    ...
    details: "Collaborating with architects, engineers, and designers to create detailed plans & blueprints."
  },
  {
    number: "02",
  ...
  return (
    ...
    >
      ...
      <div className="h-screen w-full bg-[#212529] flex flex-col  items-center">
        <div className=" relative top-[-70px]" ><div className="w-full flex flex-row gap-4 justify-center flex-wrap items-center">
          {cardData.map((card, index) => (
            <div key={index} className="card-map  w-[250px] h-[280px] flex flex-col items-center justify-between p-4 m-2">
              <div className="w-[100px] h-[100px] bg-[#212529] border border-gray-600 rounded-full flex items-center justify-center drop-shadow-2xl">
                <div className="w-[75px] h-[75px] border border-gray-600 rounded-full flex items-center justify-center hover:bg-[#F84E1D] cursor-pointer ease-in-out duration-100 drop-shadow-xl">
                  <p className="text-white text-3xl font-bold">{card.number}</p>
                </div>
              </div>
              <p className="text-white text-xl font-bold">{card.heading}</p>
              <p className="text-[#999999] text-sm text-center">{card.details}</p>
            </div>
          ))}
        </div></div>
      </div>
